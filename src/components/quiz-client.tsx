...
import { useEffect, useReducer, useRef, useCallback, useState, type ElementType } from 'react';
  ...
  return (
    <div className="grid gap-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Icon as={Clock} className="h-4 w-4" />
      ...
      <div className="grid md:grid-cols-2 gap-6 items-start">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Icon as={FileText} size={20} /> Context Data</CardTitle>
        ...
        <Card className="min-h-[280px]">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Icon as={Bot} size={20} /> AI Generated Answer</CardTitle>
            <CardDescription>The answer generated by the Large Language Model.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
             {state.status === 'idle' && (
              <div className="flex flex-col items-center justify-center text-center text-muted-foreground py-8">
                <Icon as={Lightbulb} className="w-10 h-10 mb-4 text-primary/50"/>
             ...
             {(state.status === 'solved' || state.status === 'evaluating' || state.feedback) && (
                <div className="space-y-4">
                    <div className="space-y-1">
                        <h3 className="font-semibold flex items-center gap-2 text-sm text-muted-foreground"><Icon as={Lightbulb} size={16} /> Answer</h3>
             ...
             {state.feedback && (
                <Alert variant={state.feedback === 'correct' ? 'default' : 'destructive'} className={state.feedback === 'correct' ? 'bg-green-500/10 border-green-500/40 text-green-700 dark:text-green-400' : ''}>
                    {state.feedback === 'correct' ? <Icon as={CheckCircle2} className="h-4 w-4" /> : <Icon as={XCircle} className="h-4 w-4" />}
      ...
      <div className="flex justify-end gap-4 mt-2">
        {state.status === 'idle' && (
          <Button onClick={handleSolve} disabled={isLoading} size="lg">
            {isLoading ? <Icon as={Loader2} className="mr-2 h-4 w-4 animate-spin" /> : <Icon as={Lightbulb} className="mr-2 h-4 w-4" /> }
            Let AI Solve
          </Button>
        )}
        {state.status === 'solved' && (
          <Button onClick={handleSubmit} disabled={isLoading} size="lg">
             {isLoading ? <Icon as={Loader2} className="mr-2 h-4 w-4 animate-spin" /> : <Icon as={CheckCircle2} className="mr-2 h-4 w-4" /> }
            Submit Answer
          </Button>
        )}
        {state.feedback && (
          <Button onClick={handleNextQuestion} size="lg">
            Next Question <Icon as={ChevronRight} className="ml-2 h-4 w-4" />
          </Button>
        )}
      </div>
    </div>
